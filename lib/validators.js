...
export function validateCoffee(body) {
  ...
  if (!ROAST_LEVELS.includes(roast_level) && roast_level !== '' && roast_level != undefined) errors.push('Select a roast level');
  if (!PROCESS_OPTIONS.includes(coffee_process) && coffee_process !== '' && coffee_process != undefined) errors.push('Select a process');
  ...
  if (!roaster_notes?.trim()) errors.push('Roaster notes is required');
